...
export default function QuestionFr() {
  const width = window.screen.width;
  useEffect(() => {
    window.scrollTo(0, 0);
    if (localStorage.getItem("qf-checked")) {
      setChecked(JSON.parse(localStorage.getItem("qf-checked")));
    }
    if (localStorage.getItem("qf")) {
      setInput(localStorage.getItem("qf"));
    }
    if (localStorage.getItem("qf-other")) {
  ...
  useEffect(() => {
    ...
    localStorage.setItem("qf-checked", JSON.stringify(checked));
        ...
        <div className="main">
          <div className="sticky-sub-div">
            <h2 className="percent">
              {Math.round(((100 / 39) * 37).toString())}% завершено
            </h2>
